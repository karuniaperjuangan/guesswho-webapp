<script lang="ts">
    ...
    async function onMouseEnteringBoundingBox(event: MouseEvent, face: Face) {
        ...
        if (tooltip) {
            tooltip.style.top =
                Math.min(window.innerHeight - 400, event.clientY) + "px";
            tooltip.style.left =
                Math.min(window.innerWidth - 400, event.clientX) + "px";
        }
    }
    ...
    <div class="media-preview h-fit w-fit mx-auto">
        ...
        -->
                    <div class="bounding-box h-fit w-fit relative">
                        {#each $result as face}
                            <div
                                class="bounding-box__box absolute border-2 border-primary"
                                style="top: {face.bbox.y_min *
                                    100}%; left: {face.bbox.x_min *
                                    100}%; width: {(face.bbox.x_max -
                                    face.bbox.x_min) *
                                    100}%; height: {(face.bbox.y_max -
                                    face.bbox.y_min) *
                                    100}%;"
                            >
                                <div
                                    class="h-full w-full hover:bg-slate-200 hover:opacity-25 hover:cursor-pointer transition-all"
                                    role="presentation"
                                    on:click={(element) => showModal(face)}
                                ></div>
                            </div>
                        {/each}
                        <img
                            src={previewUrl}
                            alt="Uploaded"
                            class="h-full max-h-[384px] mx-auto"
                        />
                    </div>
